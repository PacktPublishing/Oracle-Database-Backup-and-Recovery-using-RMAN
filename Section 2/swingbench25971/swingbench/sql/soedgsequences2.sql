...
begin
	declare
		...
		logon_count	number :=0;
                card_count      number :=0;
	begin
		select max(customer_id) into cust_count from customers;
		select max(order_id) into order_count from orders;
		select max(address_id) into add_count from addresses;
		select max(logon_id) into logon_count from logon;
                select max(card_id) into card_count from card_details;
